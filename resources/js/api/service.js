...
service.interceptors.response.use(
    ...
    error => {
        ...
        if (JSON.stringify(USER_NOT_FOUND) === JSON.stringify(error.response.data)) {
            ...
            } else if (error.response.data.message === 'accessory_name_of_tonnage_is_exist') {
                MakeToast({
                    variant: 'warning',
                    title: i18n.t('TOAST.TITLE.WARNING'),
                    content: i18n.t('TOAST.CONTENT.ACCESSORY_MANAGEMENT.accessory_name_of_tonnage_is_exist'),
                });
            }
        }
